...
const getovertoneglobal = (overtone) => {
    let overtonearray = overtone.split(',')
    let newovertonearray = ""
    for (let i = 0; i < overtonearray.length; i++) {
        if (i > 0 && i < overtonearray.length) {
            newovertonearray += "OR "
        }
        newovertonearray += `f_overtone LIKE '%${overtonearray[i].replaceAll("'", "").replaceAll('"', "")}%' `
    }
    if (newovertonearray.includes("NONE")) {
        newovertonearray += `OR f_overtone IS NULL OR f_overtone = '' `
    }
    // console.log(newovertonearray.toString())
...
module.exports = {
    login: async (req,res) =>{
        try{
            ...
            let contactbookquery = await QueryDB(`select id,mail,cname from contact_book where mail = '${req.body.mail}'`)
            if(!contactbookquery.data.length){
                return res.status(200).send({
                    success: false,
                    msg: "User Not Found!"
                })
            }
            let q = `select * from ccmode_setting where CustomerId = '${contactbookquery.data[0].id}';`
      ...
      let currency =  await QueryDB(currSql);
        ...
        temp.data[0].cname = contactbookquery.data[0].cname
        temp.data[0].mail = contactbookquery.data[0].mail
        return res.status(200).send({
            "success":true,
            "Customer":temp.data,
            "Currency":currency.data,
    ...
    fetchCCModeNaturalStones:async(req,res) => {
        try {
            ...
            api_currency = req.body.currency || getcurrencyandtax.data[0].api_currency
            if(getcurrencyandtax.data[0].api_taxvalue > 0){
                taxvalue = getcurrencyandtax.data[0].api_taxvalue
            }
            let natservicefee = ""
            let labservicefee = ""
            const getcountry = await QueryDB(`select country from contact_book where id = ${req.body.user_id}`)
            if (getcountry.data[0].country.toLowerCase() === "india") {
                natservicefee = "in_nat_service_fee"
                labservicefee = "in_lab_service_fee"
            }
            else if (getcountry.data[0].country.toLowerCase() === 'united kingdom' ||
                getcountry.data[0].country.toLowerCase() === 'france' ||
                getcountry.data[0].country.toLowerCase() === 'italy' ||
                getcountry.data[0].country.toLowerCase() === 'spain' ||
                getcountry.data[0].country.toLowerCase() === 'ukraine' ||
                getcountry.data[0].country.toLowerCase() === 'poland' ||
                getcountry.data[0].country.toLowerCase() === 'netherlands' ||
                getcountry.data[0].country.toLowerCase() === 'belgium' ||
                getcountry.data[0].country.toLowerCase() === 'sweden' ||
                getcountry.data[0].country.toLowerCase() === 'hungary' ||
                getcountry.data[0].country.toLowerCase() === 'switzerland' ||
                getcountry.data[0].country.toLowerCase() === 'bulgaria' ||
                getcountry.data[0].country.toLowerCase() === 'denmark' ||
                getcountry.data[0].country.toLowerCase() === 'finland' ||
                getcountry.data[0].country.toLowerCase() === 'ireland' ||
                getcountry.data[0].country.toLowerCase() === 'lithuania' ||
                getcountry.data[0].country.toLowerCase() === 'latvia' ||
                getcountry.data[0].country.toLowerCase() === 'malta' ||
                getcountry.data[0].country.toLowerCase() === 'iceland' ||
                getcountry.data[0].country.toLowerCase() === 'england' ||
                getcountry.data[0].country.toLowerCase() === 'greece' || getcountry.data[0].country.toLowerCase() === 'russian federation' || getcountry.data[0].country.toLowerCase() === 'germany') {
                    natservicefee = "eu_nat_service_fee"
                    labservicefee = "eu_lab_service_fee"
            }
            else if(getcountry.data[0].country.toLowerCase() === 'new zealand' || getcountry.data[0].country.toLowerCase() === 'australia'){
                natservicefee = "aus_nat_service_fee"
                labservicefee = "aus_lab_service_fee"
            }
            else{
                natservicefee = "global_nat_service_fee"
                labservicefee = "global_lab_service_fee"
            }
            ...
            (
                SELECT JSON_ARRAYAGG(
                    JSON_OBJECT(
                        ...
                        'markupfromrange', cm.markupfromrange,
                        'markuptorange', cm.markuptorange
                    )
                )
        ...
        FROM ccmode_rules cr where cr.user_id = ${req.body.user_id} and cr.diamond_type = 'N' `
            ...
            let finalcurrency = 1
            if (api_currency === "INR") {
                finalcurrency = getcurrency.data[0].cur_inr
        }
        if (api_currency === "USD") {
                finalcurrency = 1
        }
        ...
        if (api_currency === "SGD") {
            finalcurrency = getcurrency.data[0].cur_sgd
        }
        if (api_currency === "THB") {
            finalcurrency = getcurrency.data[0].cur_thb
        }
        if (api_currency === "MYR") {
            finalcurrency = getcurrency.data[0].cur_myr
        }
        if (api_currency === "BND") {
            finalcurrency = getcurrency.data[0].cur_bnd
        }
        if (api_currency === "DKK") {
            finalcurrency = getcurrency.data[0].cur_dkk
        }
        if (api_currency === "SEK") {
            finalcurrency = getcurrency.data[0].cur_sek
        }
        finalcurrency = Math.round(finalcurrency * 100)/100
        let fetchmarkups = JSON.parse(getrules.data[0].customer_markups)
        let fromranges = [...new Set(fetchmarkups.map(item => item.markupfromrange))]
        let toranges = [...new Set(fetchmarkups.map(item => item.markuptorange))]
        let minprice = Math.round((Math.min(...fromranges) * (1 + taxvalue/100) * finalcurrency))
        let maxprice = Math.round((Math.max(...toranges) * (1 + taxvalue/100) * finalcurrency))
        let pricefrom = Math.round((typeof(req.body.total_price_from) === "number" && req.body.total_price_from >= minprice?req.body.total_price_from:minprice)/finalcurrency * 100)/100
        let priceto = Math.round((typeof(req.body.total_price_to) === "number" && req.body.total_price_to <= maxprice?req.body.total_price_to:maxprice)/finalcurrency * 100)/100
        // if(pricefrom < getrules.data[0].total_price_from){
        //     pricefrom = getrules.data[0].total_price_from
        // }
        // if(priceto > getrules.data[0].total_price_to){
        //     priceto = getrules.data[0].total_price_to
        // }
        if(getcurrencyandtax.data[0].api_taxvalue){
            pricefrom = pricefrom/(1 + taxvalue/100)
            priceto = priceto/(1 + taxvalue/100)
        }
        //console.log(pricefrom,priceto,"After Tax Deduct")
        let customermarkups = JSON.parse(getrules.data[0].customer_markups)
        const fetchservicefees = await QueryDB(`select * from ${natservicefee}`)
        if(getrules.data[0].markupname === "Price"){
            let markuprangefrom = customermarkups.find(val => pricefrom >= val.markupfromrange && pricefrom <= val.markuptorange)
        let markuprangeto = customermarkups.find(val => priceto >= val.markupfromrange && priceto <= val.markuptorange)
        // console.log(markuprangefrom,"markuprangefrom")
        // console.log(markuprangeto,"markuprangeto")
        if(markuprangefrom && markuprangefrom.markuptype === "Absolute"){
            pricefrom = pricefrom - markuprangefrom.markupvalue
        }
        if(markuprangeto && markuprangeto.markuptype === "Absolute"){
            priceto = priceto - markuprangeto.markupvalue
        }
        if(markuprangefrom && markuprangefrom.markuptype === "Percentage"){
            pricefrom = pricefrom/(1 + markuprangefrom.markupvalue/100)
        }
        if(markuprangeto && markuprangeto.markuptype === "Percentage"){
            priceto = priceto/(1 + markuprangeto.markupvalue/100)
        }
        //console.log(pricefrom,priceto,"After Markup Deduct")
        // const fetchservicefees = await QueryDB(`select * from nat_service_fee`)
        let minnsf = fetchservicefees.data.find(value => pricefrom >= value.markup_min_range && pricefrom <= value.markup_max_range)
        let maxnsf = fetchservicefees.data.find(value => priceto >= value.markup_min_range && priceto <= value.markup_max_range)
        // console.log(minnsf,"minnsf")
        // console.log(maxnsf,"maxnsf")
        if(minnsf && minnsf.type === "dollar"){
            pricefrom = pricefrom - minnsf.charges
        }
        if(maxnsf && maxnsf.type === "dollar"){
            priceto = priceto - maxnsf.charges
        }
        if(minnsf && minnsf.type === "perc"){
            pricefrom = pricefrom/(1 + minnsf.charges/100)
        }
        if(maxnsf && maxnsf.type === "perc"){
            priceto = priceto/(1 + maxnsf.charges/100)
        }
        //console.log(Math.round(pricefrom * 100)/100,Math.round(priceto * 100)/100,"After Service Fee Deduct")
        }
        let caratfrom = typeof(req.body.min_carat) === "number" && req.body.min_carat >=  getrules.data[0].min_carat?req.body.min_carat:getrules.data[0].min_carat
        let caratto = typeof(req.body.max_carat) === "number" && req.body.max_carat <=  getrules.data[0].max_carat?req.body.max_carat:getrules.data[0].max_carat
        let caratsubquery = ``
        if(getrules.data[0].markupname === "Carat"){
            function filterByRange(array, minRange, maxRange) {
                return array.filter(obj => 
                  (obj.fromrange >= minRange && obj.fromrange <= maxRange) || 
                  (obj.torange >= minRange && obj.torange <= maxRange) ||
                  (obj.fromrange < minRange && obj.torange > maxRange)
                );
              }
            let activatedcaratslots = filterByRange(customermarkups,caratfrom,caratto)
            let activatedcaratranges = []
            for(let i = 0 ; i < activatedcaratslots.length; i++){
                if(activatedcaratslots.length === 1){
                    activatedcaratranges.push({
                        ...activatedcaratslots[i],
                        fromrange:caratfrom,
                        torange:caratto
                    })
                }
                ...
                else if(i+1 === activatedcaratslots.length){
                    activatedcaratranges.push({
                        ...activatedcaratslots[i],
                        torange:caratto
                    })
                    ...
                    activatedcaratranges.push({...activatedcaratslots[i]})
                }
            }
            // console.log(activatedcaratranges,"activatedcaratranges")
            let finalranges = activatedcaratranges.map(value => {
                let markupfromrange = pricefrom
                let markuptorange = priceto
                let finalcnetfrom = 0
                let finalcnetto = 0
                if(value.markuptype === "Absolute"){
                    markupfromrange = markupfromrange - value.markupvalue
                    markuptorange = markuptorange - value.markupvalue
                }
                if(value.markuptype === "Percentage"){
                    markupfromrange = markupfromrange/(1 + value.markupvalue/100)
                    markuptorange = markuptorange/(1 + value.markupvalue/100)
                }
                let minnsf = fetchservicefees.data.find(value => markupfromrange >= value.markup_min_range && markupfromrange <= value.markup_max_range)
                let maxnsf = fetchservicefees.data.find(value => markuptorange >= value.markup_min_range && markuptorange <= value.markup_max_range)
                // console.log(minnsf,"minnsf")
                // console.log(maxnsf,"maxnsf")
                if(minnsf && minnsf.type === "dollar"){
                    finalcnetfrom = Math.round((markupfromrange - minnsf.charges) * 100)/100
                }
                if(maxnsf && maxnsf.type === "dollar"){
                    finalcnetto = Math.round((markuptorange - maxnsf.charges) * 100)/100
                }
                if(minnsf && minnsf.type === "perc"){
                    finalcnetfrom = Math.round(markupfromrange/(1 + minnsf.charges/100) * 100)/100
                }
                if(maxnsf && maxnsf.type === "perc"){
                    finalcnetto = Math.round(markuptorange/(1 + maxnsf.charges/100) * 100)/100
                }
                caratsubquery += `(C_Weight >= ${value.fromrange} and C_Weight <= ${value.torange} and C_NetD >= ${finalcnetfrom} and C_NetD <= ${finalcnetto})`
                return {
                    ...value,
                    markupfromrange:markupfromrange,
                    markuptorange:markuptorange,
                    finalcnetfrom:finalcnetfrom,
                    finalcnetto:finalcnetto
                }
            })
            // console.log(finalranges,"finalranges")
            caratsubquery = caratsubquery.replaceAll(")(",") OR (")
            //console.log(caratsubquery,"caratsubquery")
        ...
        let shapetemp = ""
            for(let i = 0; i < getrules.data.length;i++){
                ...
                `(SELECT charges FROM ${natservicefee} WHERE (C_NetD BETWEEN min_range and max_range)) as lab_charges, ` +
                ...
                `(SELECT type FROM ${natservicefee} WHERE (C_NetD BETWEEN min_range and max_range)) as lab_type, (SELECT caratprice from rap_master WHERE` +
                ...
                `(SELECT india_fee FROM supplier WHERE id = sup_id) as india_fee,` +
                // "(SELECT count(*) as ct FROM `conform_goods` WHERE `certi_no` = diamond_master.Certi_NO AND `is_hold` = 0) as ct," +
                //"(select show_supplier from contact_book where `id` = " + req.body.user_id + ")as show_supplier," +
                `(SELECT type FROM ${natservicefee} WHERE (C_NetD BETWEEN min_range and max_range)) as lab_type, (SELECT caratprice from rap_master WHERE` +
                " IF(diamond_master.`C_Shape` = 'ROUND', shape = 'ROUND', shape != 'ROUND') AND color = diamond_master.`C_Color` AND clarity = IF(diamond_master.`C_Clarity` = 'FL', 'IF', diamond_master.`C_Clarity`)" +
                ...
                if(getrules.data[i].markupname === "Price"){
                    newnaturalsqlquery += `${condition} C_Weight >= ${parseFloat(caratfrom)} `
                    newnaturalsqlquery += `${condition} C_Weight <= ${parseFloat(caratto)} `
                }
                if(getrules.data[i].markupname === "Carat"){
                    newnaturalsqlquery += `${condition} (${caratsubquery}) `
                    condition = "AND"
                }
                //Older 
                ...
                //New Harshil Sir Logic
                if(getrules.data[i].markupname === "Price"){
                newnaturalsqlquery += `${condition} C_NetD >= ${Math.round(pricefrom * 100)/100} `
                newnaturalsqlquery += `${condition} C_NetD <= ${Math.round(priceto * 100)/100} `
                }
                ...
                if(typeof(req.body.ratiomin) === "number" && typeof(getrules.data[i].ratiomin) === "number" && typeof(req.body.ratiomax) === "number" && typeof(getrules.data[i].ratiomax) === "number" && typeof(req.body.ratiomin) === "number" && req.body.ratiomin >= getrules.data[i].ratiomin && req.body.ratiomin <= getrules.data[i].ratiomax && typeof(req.body.ratiomax) === "number" && req.body.ratiomax <= getrules.data[i].ratiomax && req.body.ratiomax >= getrules.data[i].ratiomin){
                    ...
                    newnaturalsqlquery += `${condition} COALESCE(Ratio, 0) between ${parseFloat(req.body.ratiomin)} and ${parseFloat(req.body.ratiomax)} `
                    condition = "AND"
                }
                else{
                    if(typeof(getrules.data[i].ratiomin) === "number" && typeof(getrules.data[i].ratiomax) === "number"){
                        ...
                        newnaturalsqlquery += `${condition} COALESCE(Ratio, 0) between ${parseFloat(getrules.data[i].ratiomin)} and ${parseFloat(getrules.data[i].ratiomax)} `
                        condition = "AND"
                    }
                    else{
                ...
                if(req.body.fancy_color_diamond&& req.body.fancy_color_diamond.toUpperCase() === "YES"){
                    ...
                    if(req.body.fancy_overtone && Array.isArray(req.body.fancy_overtone)){
                        if(getrules.data[i].diamondfancy_overtone){
                            ...
                            getexistingshapes = getovertoneglobal(getexistingshapes)
                            if(getexistingshapes){
                                naturalsqlquery += `AND (${getexistingshapes}) `
                                newnaturalsqlquery += `${condition} (${getexistingshapes}) `
                                condition = "AND"
                            }
                            else{
                                naturalsqlquery += `AND (${getovertoneglobal(getrules.data[i].diamondfancy_overtone.split(',').map(v => JSON.stringify(v)).join(','))}) `
                                newnaturalsqlquery += `${condition} (${getovertoneglobal(getrules.data[i].diamondfancy_overtone.split(',').map(v => JSON.stringify(v)).join(','))}) `
                                condition = "AND"
                            }
                        }
                        else{
                            naturalsqlquery += `AND (${getovertoneglobal(req.body.fancy_overtone.map(v => JSON.stringify(v)).join(','))}) `
                            newnaturalsqlquery += `${condition} (${getovertoneglobal(req.body.fancy_overtone.map(v => JSON.stringify(v)).join(','))}) `
                            condition = "AND"
                        }
                    }
                    else{
                ...
                for (let j = 0; j < splitfilters.length; j++) {
                    ...
                    if (splitfilters[j] === "IMAGE AND VIDEO") {
                        naturalsqlquery += `AND aws_image <> '0' AND aws_image <> '' AND video <> '0' AND video <> '' `
                        newnaturalsqlquery += `AND aws_image <> '0' AND aws_image <> '' AND video <> '0' AND video <> '' `
                        condition = "AND"
                    }
                    if (splitfilters[j] === "IMAGE OR VIDEO") {
                        naturalsqlquery += `AND (aws_image <> '0' AND aws_image <> '' OR video <> '0' AND video <> '') `
                        newnaturalsqlquery += `AND (aws_image <> '0' AND aws_image <> '' OR video <> '0' AND video <> '') `
                        condition = "AND"
                    }
            ...
            if(req.body.sort_field === "carat"){
                ...
                if(req.body.sort_order.toUpperCase() === "ASC"){
                    sortquery += `ORDER BY FIELD(C_Clarity, 'FL', 'IF', 'VVS1', 'VVS2', 'VS1', 'VS2', 'SI1', 'SI2', 'SI3', 'I1', 'I2', 'I3') `                    
                }
                else{
                    sortquery += `ORDER BY FIELD(C_Clarity, 'I3', 'I2', 'I1', 'SI3', 'SI2', 'SI1', 'VS2', 'VS1', 'VVS2', 'VVS1', 'IF', 'FL') `
                }
            }
            ...
            for (let i = 0; i < fetchdata.data.length; i++) {
                let calculateprice = CalculatePrice(fetchdata.data[i],getcountry.data[0].country)
                let markupprice = 0
                ...
                if (fetchdata.data[i].markupcurr === "INR") {
                        markupcurrencyvalue = getcurrency.data[0].cur_inr
                }
                ...
                if (fetchdata.data[i].markupcurr === "SGD") {
                    markupcurrencyvalue = getcurrency.data[0].cur_sgd
            }
                if (fetchdata.data[i].markupcurr === "THB") {
                        markupcurrencyvalue = getcurrency.data[0].cur_thb
                }
                if (fetchdata.data[i].markupcurr === "MYR") {
                        markupcurrencyvalue = getcurrency.data[0].cur_myr
                }
                if (fetchdata.data[i].markupcurr === "BND") {
                        markupcurrencyvalue = getcurrency.data[0].cur_bnd
                }
                if (fetchdata.data[i].markupcurr === "DKK") {
                        markupcurrencyvalue = getcurrency.data[0].cur_dkk
                }
                if (fetchdata.data[i].markupcurr === "SEK") {
                        markupcurrencyvalue = getcurrency.data[0].cur_sek
                }
                    ...
                    if(getmarkup){
                        if(getmarkup.markuptype === "Absolute"){
                            if(calculateprice.total_our_price){
                                markupprice = Math.round(((Math.round(calculateprice.total_our_price * 100)/100) + getmarkup.markupvalue)*100)/100 
                                markupprice = (markupprice + (markupprice * taxvalue/100)) * Math.round(markupcurrencyvalue*100)/100
                                ...
                                let FinalObject = {
                                    ...
                                    markupdollpercar:markupdollpercar
                                }
                                if(req.body.fancy_color_diamond && req.body.fancy_color_diamond.toUpperCase() === "YES"){
                        ...
                        if(getmarkup.markuptype === "Percentage"){
                            if(calculateprice.total_our_price){
                                markupprice = Math.round(((Math.round(calculateprice.total_our_price * 100)/100  ) + (Math.round(calculateprice.total_our_price * 100)/100 * getmarkup.markupvalue/100 ))* 100)/100
                                markupprice = (markupprice + (markupprice * taxvalue/100)) * Math.round(markupcurrencyvalue*100)/100
                                markupdollpercar = Math.round(markupprice/fetchdata.data[i].C_Weight * 100)/100
                                let FinalObject = {
                                    //SUPPLIER_NAME: fetchdata.data[i].C_Name,
                ...
                if(fetchdata.data[i].markupname === "Price"){
                    const getmarkup = JSON.parse(fetchdata.data[i].customer_markups).find(val => val.rule_id.toString() === fetchdata.data[i].rule_id.toString() && (Math.round(calculateprice.total_our_price * 100)/100 ) >= val.fromrange && (Math.round(calculateprice.total_our_price * 100)/100 ) <= val.torange)
                        if(getmarkup){
                            if(getmarkup.markuptype === "Absolute"){
                                if(calculateprice.total_our_price){
                                    markupprice = Math.round(((Math.round(calculateprice.total_our_price * 100)/100) + getmarkup.markupvalue)*100)/100 
            ...
            if(req.body.sort_field === "price"){
                if(req.body.sort_order.toUpperCase() === "ASC"){
                    finaloutput = finaloutput.sort((a, b) => a.TOTAL_PRICE - b.TOTAL_PRICE);
                }
                if(req.body.sort_order.toUpperCase() === "DESC"){
                    finaloutput = finaloutput.sort((a, b) => b.TOTAL_PRICE - a.TOTAL_PRICE);;
                }
            }
            else if(!req.body.sort_field){
                finaloutput = finaloutput.sort((a, b) => a.TOTAL_PRICE - b.TOTAL_PRICE);
            } 
            let finalObj = {
                success:true,
    ...
    fetchCCModeLabStones:async(req,res) => {
        ...
        let pricefrom = Math.round((typeof(req.body.total_price_from) === "number" && req.body.total_price_from >= minprice?req.body.total_price_from:minprice)/finalcurrency * 100)/100
        let priceto = Math.round((typeof(req.body.total_price_to) === "number" && req.body.total_price_to <= maxprice?req.body.total_price_to:maxprice)/finalcurrency * 100)/100
        // console.log(pricefrom,priceto,"After Currency Divide")
        // if(pricefrom < getrules.data[0].lab_total_price_from){
        //     pricefrom = getrules.data[0].lab_total_price_from
        // }
        // if(priceto > getrules.data[0].lab_total_price_to){
        //     priceto = getrules.data[0].lab_total_price_to
        // }
        if(getcurrencyandtax.data[0].api_taxvalue){
            pricefrom = pricefrom/(1 + taxvalue/100)
            priceto = priceto/(1 + taxvalue/100)
        ...
        //console.log(pricefrom,priceto,"After Tax Deduct")
        let customermarkups = JSON.parse(getrules.data[0].customer_markups)
        const fetchservicefees = await QueryDB(`select * from ${labservicefee}`)
        if(getrules.data[0].markupname === "Price"){
            let markuprangefrom = customermarkups.find(val => pricefrom >= val.markupfromrange && pricefrom <= val.markuptorange)
        let markuprangeto = customermarkups.find(val => priceto >= val.markupfromrange && priceto <= val.markuptorange)
        // console.log(markuprangefrom,"markuprangefrom")
        ...
        let caratfrom = typeof(req.body.min_carat) === "number" && req.body.min_carat >= getrules.data[0].lab_min_carat?req.body.min_carat:getrules.data[0].lab_min_carat
        let caratto = typeof(req.body.max_carat) === "number" && req.body.max_carat <= getrules.data[0].lab_max_carat?req.body.max_carat:getrules.data[0].lab_max_carat
        let caratsubquery = ``
        if(getrules.data[0].markupname === "Carat"){
            function filterByRange(array, minRange, maxRange) {
                return array.filter(obj => 
        ...
        let shapetemp = ""
            for(let i = 0; i < getrules.data.length;i++){
                ...
                let labsqlquery = `SELECT id,Loat_NO,diamond_type,availability,C_Shape,C_Weight,C_Color,C_Clarity,C_Cut,C_Polish,C_Symmetry,C_Fluorescence,Lab,Certi_NO,Certificate_link,certificate_download_check,C_Length,C_Width,C_Depth,Location,City,country,brown,green,Milky,shade,luster,EyeC,HNA,C_DefthP,C_TableP,Crn_Ag,Crn_Ht,Pav_Ag,Pav_Dp,C_Discount,C_Rap,O_Rate,C_Rate,C_NetD,Key_Symbols,image_d_status,aws_image,image,video,heart,aws_heart,arrow,aws_arrow,asset,aws_asset,canada_mark,cutlet,culet_condition,gridle,gridle_per,girdle_thin,girdle_thick,c_type,f_color,f_overtone,f_intensity,supplier_comments,extra_string1,extra_string2,extra_integer1,report_comments,Status,hold_for,hold_date,hold_status,created_date,is_delete, C_Name, lab_treat, ${getrules.data[i].markupperc} as markupperc, '${api_currency}' as markupcurr, ${getrules.data[i].markupdollar} as markupdollar, ${getrules.data[i].rule_id} as rule_id, '${getrules.data[i].markupname}' as markupname,` +
                    `(SELECT charges FROM ${labservicefee} WHERE (C_NetD BETWEEN min_range and max_range)) as lab_charges, ` +
                    // "(SELECT count(*) as ct FROM `conform_goods` WHERE `certi_no` = lab_diamond_master.Certi_NO AND `is_hold` = 0) as ct," +
                    //"(select show_supplier from contact_book where `id` = " + req.body.user_id + ")as show_supplier," +
                    `(SELECT type FROM ${labservicefee} WHERE (C_NetD BETWEEN min_range and max_range)) as lab_type, (SELECT caratprice from rap_master WHERE` +
                ...
                `(SELECT india_fee FROM supplier WHERE id = sup_id) as india_fee,` +
                    ...
                    if(getrules.data[i].markupname === "Price"){
                        newlabsqlquery += `${condition} C_Weight >= ${parseFloat(caratfrom)} `
                        newlabsqlquery += `${condition} C_Weight <= ${parseFloat(caratto)} `
                    }
                    if(getrules.data[i].markupname === "Carat"){
                        newlabsqlquery += `${condition} (${caratsubquery}) `
                        condition = "AND"
                    }
                    //Older
                    ...
                    //New Harshil Sir Logic
                if(getrules.data[i].markupname === "Price"){
                    newlabsqlquery += `${condition} C_NetD >= ${Math.round(pricefrom * 100)/100} `
                    newlabsqlquery += `${condition} C_NetD <= ${Math.round(priceto * 100)/100} `
                    }
                    ...
                    if(typeof(req.body.ratiomin) === "number" && typeof(getrules.data[i].labratiomin) === "number" && typeof(req.body.ratiomax) === "number" && typeof(getrules.data[i].labratiomax) === "number" && typeof(req.body.ratiomin) === "number" && req.body.ratiomin >= getrules.data[i].labratiomin && req.body.ratiomin <= getrules.data[i].labratiomax && typeof(req.body.ratiomax) === "number" && req.body.ratiomax <= getrules.data[i].labratiomax && req.body.ratiomax >= getrules.data[i].labratiomin){
                        ...
                        newlabsqlquery += `${condition} COALESCE(Ratio, 0) between ${parseFloat(req.body.ratiomin)} and ${parseFloat(req.body.ratiomax)} `
                        condition = "AND"
                    }
                    else{
                        if(typeof(getrules.data[i].labratiomin) === "number" && typeof(getrules.data[i].labratiomax) === "number"){
                            ...
                            newlabsqlquery += `${condition} COALESCE(Ratio, 0) between ${parseFloat(getrules.data[i].labratiomin)} and ${parseFloat(getrules.data[i].labratiomax)} `
                            condition = "AND"
                        }
                        else{
                            if(typeof(req.body.ratiomin) === "number" && typeof(req.body.ratiomax) === "number"){
                    ...
                    if(req.body.fancy_color_diamond&& req.body.fancy_color_diamond.toUpperCase() === "YES"){
                        ...
                        if(req.body.fancy_overtone && Array.isArray(req.body.fancy_overtone)){
                            if(getrules.data[i].lab_fancy_overtone){
                                ...
                                getexistingshapes = getovertoneglobal(getexistingshapes)
                                if(getexistingshapes){
                                    labsqlquery += `AND (${getexistingshapes}) `
                                    newlabsqlquery += `${condition} (${getexistingshapes}) `
                                    condition = "AND"
                                }
                                else{
                                    labsqlquery += `AND (${getovertoneglobal(getrules.data[i].lab_fancy_overtone.split(',').map(v => JSON.stringify(v)).join(','))}) `
                                    newlabsqlquery += `${condition} (${getovertoneglobal(getrules.data[i].lab_fancy_overtone.split(',').map(v => JSON.stringify(v)).join(','))}) `
                                    condition = "AND"
                                }
                            }
                            else{
                                labsqlquery += `AND (${getovertoneglobal(req.body.fancy_overtone.map(v => JSON.stringify(v)).join(','))}) ` 
                                newlabsqlquery += `${condition} (${getovertoneglobal(req.body.fancy_overtone.map(v => JSON.stringify(v)).join(','))}) ` 
                                condition = "AND"
                            }
                        }
                        else{
                    ...
                    for (let j = 0; j < splitfilters.length; j++) {
                        ...
                        if (splitfilters[j] === "IMAGE AND VIDEO") {
                            labsqlquery += `AND aws_image <> '0' AND aws_image <> '' AND video <> '0' AND video <> '' `
                            newlabsqlquery += `AND aws_image <> '0' AND aws_image <> '' AND video <> '0' AND video <> '' `
                            condition = "AND"
                        }
                        if (splitfilters[j] === "IMAGE OR VIDEO") {
                            labsqlquery += `AND (aws_image <> '0' AND aws_image <> '' OR video <> '0' AND video <> '') `
                            newlabsqlquery += `AND (aws_image <> '0' AND aws_image <> '' OR video <> '0' AND video <> '') `
                            condition = "AND"
                        }
                        if (splitfilters[j] === "HA") {
    ...
    fetchCCModeDiamondDetail:async(req,res) => {
        try {
            ...
            if(req.body.diamond_type !== "N" && req.body.diamond_type !== "L" && req.body.diamond_type !== "NF" && req.body.diamond_type !== "LF"){
                return res.send("Please Provide Valid Diamond Type N,L,NF & LF")
            }
            let taxvalue = 0
            let api_currency = ""
            const getcurrencyandtax = await QueryDB(`select Currency as api_currency,TaxName as api_taxname,TaxValue as api_taxvalue from ccmode_setting where CustomerId = ${req.body.user_id}`)
        ...
        if(req.body.diamond_type === "N"){
            rulequery += "and cr.naturaldiamond = 1"
        }
        if(req.body.diamond_type === "NF"){
            rulequery += "and cr.naturalfancydiamond = 1"
        }
        if(req.body.diamond_type === "L"){
            rulequery += "and cr.labdiamond = 1"
        }
        ...
        let pricefrom = Math.round((typeof(req.body.total_price_from) === "number" && req.body.total_price_from >= minprice?req.body.total_price_from:minprice)/finalcurrency * 100)/100
        let priceto = Math.round((typeof(req.body.total_price_to) === "number" && req.body.total_price_to <= maxprice?req.body.total_price_to:maxprice)/finalcurrency * 100)/100
        // if(req.body.diamond_type === "L" || req.body.diamond_type === "LF"){
        //     pricefrom = Math.round((typeof(req.body.total_price_from) === "number"?req.body.total_price_from:getrules.data[0].lab_total_price_from) * 100)/100
        //     priceto = Math.round((typeof(req.body.total_price_to) === "number"?req.body.total_price_to:getrules.data[0].lab_total_price_to) * 100)/100
        // }
        //console.log(pricefrom,priceto,"After Currency Divide")
        if(getcurrencyandtax.data[0].api_taxvalue){
            pricefrom = pricefrom/(1 + taxvalue/100)
        ...
        //console.log(pricefrom,priceto,"After Tax Deduct")
        let customermarkups = JSON.parse(getrules.data[0].customer_markups)
        let fetchservicefees = ""
        if(req.body.diamond_type === "L" || req.body.diamond_type === "LF"){
            fetchservicefees = await QueryDB(`select * from lab_service_fee`)
            //console.log(fetchservicefees,`select * from lab_service_fee`)
        }
        else{
            fetchservicefees = await QueryDB(`select * from nat_service_fee`)
            //console.log(fetchservicefees,`select * from nat_service_fee`)
        }
        if(getrules.data[0].markupname === "Price"){
            let markuprangefrom = customermarkups.find(val => pricefrom >= val.markupfromrange && pricefrom <= val.markuptorange)
        let markuprangeto = customermarkups.find(val => priceto >= val.markupfromrange && priceto <= val.markuptorange)
        ...
        let caratfrom = typeof(req.body.min_carat) === "number"?req.body.min_carat:getrules.data[0].min_carat
        let caratto = typeof(req.body.max_carat) === "number"?req.body.max_carat:getrules.data[0].max_carat
        if(req.body.diamond_type === "L" || req.body.diamond_type === "LF"){
            caratfrom = typeof(req.body.min_carat) === "number"?req.body.min_carat:getrules.data[0].lab_min_carat
            caratto = typeof(req.body.max_carat) === "number"?req.body.max_carat:getrules.data[0].lab_max_carat
        }
        let caratsubquery = ``
        if(getrules.data[0].markupname === "Carat"){
            function filterByRange(array, minRange, maxRange) {
        ...
        let shapetemp = ""
            for(let i = 0; i < getrules.data.length;i++){
                ...
                if(getrules.data[i].markupname === "Carat"){
                    naturalsqlquery += `AND (${caratsubquery}) `
                }
                // if(typeof(getrules.data[i].total_price_from) === "number" && typeof(getrules.data[i].total_price_to) === "number"){
                //     naturalsqlquery += `AND (CASE WHEN (SELECT type FROM nat_service_fee WHERE (C_NetD BETWEEN min_range and max_range)) = 'Dollar' THEN (((C_Rate * C_Weight) + (SELECT charges FROM nat_service_fee WHERE (C_NetD BETWEEN min_range and max_range))) * ${finalcurrency}) ELSE (((C_Rate * C_Weight) + (C_Rate * C_Weight * (SELECT charges FROM nat_service_fee WHERE (C_NetD BETWEEN min_range and max_range))/100)) * ${finalcurrency}) END) >= ${parseFloat(getrules.data[i].total_price_from)} `
                //     naturalsqlquery += `AND (CASE WHEN (SELECT type FROM nat_service_fee WHERE (C_NetD BETWEEN min_range and max_range)) = 'Dollar' THEN (((C_Rate * C_Weight) + (SELECT charges FROM nat_service_fee WHERE (C_NetD BETWEEN min_range and max_range))) * ${finalcurrency}) ELSE (((C_Rate * C_Weight) + (C_Rate * C_Weight * (SELECT charges FROM nat_service_fee WHERE (C_NetD BETWEEN min_range and max_range))/100)) * ${finalcurrency}) END) <= ${parseFloat(getrules.data[i].total_price_to)} `
                // }
                if(getrules.data[i].markupname === "Price"){
                    naturalsqlquery += `AND C_NetD >= ${Math.round(pricefrom * 100)/100} `
                    naturalsqlquery += `AND C_NetD <= ${Math.round(priceto * 100)/100} `
                    }
                    ...
                    if(typeof(getrules.data[i].ratiomin) === "number" && typeof(getrules.data[i].ratiomax) === "number"){
                        // naturalsqlquery += `AND C_Shape <> "ROUND" and (C_Shape <> "HEART" and IF(C_Shape = "HEART",C_Length > C_Width , C_Length/C_Width ) between ${parseFloat(getrules.data[i].ratiomin)} and ${parseFloat(getrules.data[i].ratiomax)}) `
                        naturalsqlquery += `AND COALESCE(Ratio, 0) between ${parseFloat(getrules.data[i].ratiomin)} and ${parseFloat(getrules.data[i].ratiomax)} `
                    }
                    if(req.body.fancy_color && Array.isArray(req.body.fancy_color)){
                        if(getrules.data[i].diamondfancy_color){
                            let getexistingshapes = GetExisting(getrules.data[i].diamondfancy_color.split(','),req.body.fancy_color).map(v => JSON.stringify(v)).join(',')
                ...
                `(SELECT india_fee FROM supplier WHERE id = sup_id) as india_fee,` +
                    ...
                    //New Harshil Sir Logic
                    if(getrules.data[i].markupname === "Price"){
                    labsqlquery += `AND C_NetD >= ${Math.round(pricefrom * 100)/100} `
                    labsqlquery += `AND C_NetD <= ${Math.round(priceto * 100)/100} `
                    }
                        // if(req.body.min_dollarperct && req.body.max_dollarperct){
                        ...
                        if(typeof(getrules.data[i].labratiomin) === "number" && typeof(getrules.data[i].labratiomax) === "number"){
                            // labsqlquery += `AND C_Shape <> "ROUND" and (C_Shape <> "HEART" and IF(C_Shape = "HEART",C_Length > C_Width , C_Length/C_Width ) between ${parseFloat(getrules.data[i].labratiomin)} and ${parseFloat(getrules.data[i].labratiomax)}) `
                            labsqlquery += `AND COALESCE(Ratio, 0) between ${parseFloat(getrules.data[i].labratiomin)} and ${parseFloat(getrules.data[i].labratiomax)} `      
                        }
                        if(req.body.fancy_color && Array.isArray(req.body.fancy_color)){
                            if(getrules.data[i].lab_fancy_color){
                                let getexistingshapes = GetExisting(getrules.data[i].lab_fancy_color.split(','),req.body.fancy_color).map(v => JSON.stringify(v)).join(',')
    ...
    fetchCCDiamondCount:async(req,res) => {
        ...
        const fetchservicefees = await QueryDB(`select * from ${natservicefee}`)
            for(let i = 0; i < getrules.data.length;i++){
                ...
                // let getsupplierrule = fetchsupplier.filter(val => val.rule_id === getrules.data[i].rule_id && val.on_off === 1)
                // let suppliers = [...new Set(getsupplierrule.map(item => item.supplier_name))].toString()
                let fetchmarkups = JSON.parse(getrules.data[i].customer_markups)
                    let fromranges = [...new Set(fetchmarkups.map(item => item.markupfromrange))]
                    let toranges = [...new Set(fetchmarkups.map(item => item.markuptorange))]
                    let minprice = Math.round((Math.min(...fromranges) * (1 + taxvalue / 100) * finalcurrency))
                    let maxprice = Math.round((Math.max(...toranges) * (1 + taxvalue / 100) * finalcurrency))
                    ...
                    let priceto = Math.round((typeof (req.body.total_price_to) === "number" && req.body.total_price_to <= maxprice ? req.body.total_price_to : maxprice) / finalcurrency * 100) / 100
                    //console.log(pricefrom,priceto,"After Currency Divide")
                    // if(pricefrom < getrules.data[i].total_price_from){
                    //     pricefrom = getrules.data[i].total_price_from
                    // }
                    // if(priceto > getrules.data[i].total_price_to){
                    //     priceto = getrules.data[i].total_price_to
                    // }
                    if (getcurrencyandtax.data[0].api_taxvalue) {
                        pricefrom = pricefrom / (1 + taxvalue / 100)
                        priceto = priceto / (1 + taxvalue / 100)
                    ...
                    //console.log(pricefrom,priceto,"After Tax Deduct")
                    let customermarkups = JSON.parse(getrules.data[i].customer_markups)
                    ...
                    let caratfrom = typeof (req.body.min_carat) === "number" && req.body.min_carat >= getrules.data[i].min_carat ? req.body.min_carat : getrules.data[i].min_carat
                    let caratto = typeof (req.body.max_carat) === "number" && req.body.max_carat <= getrules.data[i].max_carat ? req.body.max_carat : getrules.data[i].max_carat
                    let caratsubquery = ``
                    if (getrules.data[i].markupname === "Carat") {
                        function filterByRange(array, minRange, maxRange) {
                            return array.filter(obj =>
                        ...
                        caratsubquery = caratsubquery.replaceAll(")(", ") OR (")
                        if (!caratsubquery) {
                            caratsubquery = `(C_Weight >= 0 and C_Weight <= 0 and C_NetD >= 0 and C_NetD <= 0)`
                        }
                        //console.log(caratsubquery,"caratsubquery")
                        // return res.send("DONE")
                    }
            ...
            const fetchservicefees = await QueryDB(`select * from ${labservicefee}`)
                for(let i = 0; i < getrules.data.length;i++){
                    ...
                    let caratfrom = typeof (req.body.min_carat) === "number" && req.body.min_carat >= getrules.data[i].lab_min_carat ? req.body.min_carat : getrules.data[i].lab_min_carat
                    let caratto = typeof (req.body.max_carat) === "number" && req.body.max_carat <= getrules.data[i].lab_max_carat ? req.body.max_carat : getrules.data[i].lab_max_carat
                    let caratsubquery = ``
                    if (getrules.data[i].markupname === "Carat") {
                        function filterByRange(array, minRange, maxRange) {
                            return array.filter(obj =>
                    ...
                    let newlabsqlquery = `SELECT id,Loat_NO,diamond_type,availability,C_Shape,C_Weight,C_Color,C_Clarity,C_Cut,C_Polish,C_Symmetry,C_Fluorescence,Lab,Certi_NO,Certificate_link,certificate_download_check,C_Length,C_Width,C_Depth,Location,City,country,brown,green,Milky,shade,luster,EyeC,HNA,C_DefthP,C_TableP,Crn_Ag,Crn_Ht,Pav_Ag,Pav_Dp,C_Discount,C_Rap,O_Rate,C_Rate,C_NetD,Key_Symbols,image_d_status,aws_image,image,video,heart,aws_heart,arrow,aws_arrow,asset,aws_asset,canada_mark,cutlet,culet_condition,gridle,gridle_per,girdle_thin,girdle_thick,c_type,f_color,f_overtone,f_intensity,supplier_comments,extra_string1,extra_string2,extra_integer1,report_comments,Status,hold_for,hold_date,hold_status,created_date,is_delete, C_Name, lab_treat, ${getrules.data[i].markupperc} as markupperc, '${api_currency}' as markupcurr, ${getrules.data[i].markupdollar} as markupdollar, ${getrules.data[i].rule_id} as rule_id, '${getrules.data[i].markupname}' as markupname,video_status, '${getrules.data[i].customer_markups}' as customer_markups, Ratio ` +
                        ...
                        if (typeof (req.body.ratiomin) === "number" && typeof (getrules.data[i].labratiomin) === "number" && typeof (req.body.ratiomax) === "number" && typeof (getrules.data[i].labratiomax) === "number" && typeof (req.body.ratiomin) === "number" && req.body.ratiomin >= getrules.data[i].labratiomin && req.body.ratiomin <= getrules.data[i].labratiomax && typeof (req.body.ratiomax) === "number" && req.body.ratiomax <= getrules.data[i].labratiomax && req.body.ratiomax >= getrules.data[i].labratiomin) {
                            ...
                            if (typeof (getrules.data[i].labratiomin) === "number" && typeof (getrules.data[i].labratiomax) === "number") {
                                labsqlquery += `AND C_Shape <> "ROUND" and (C_Shape <> "HEART" and IF(C_Shape = "HEART",C_Length > C_Width , C_Length/C_Width ) between ${parseFloat(getrules.data[i].labratiomin)} and ${parseFloat(getrules.data[i].labratiomax)}) `
                                // newlabsqlquery += `${condition} C_Shape <> "ROUND" and (C_Shape <> "HEART" and IF(C_Shape = "HEART",C_Length > C_Width , C_Length/C_Width ) between ${parseFloat(getrules.data[i].labratiomin)} and ${parseFloat(getrules.data[i].labratiomax)}) `
                                newlabsqlquery += `${condition} (IF(C_Shape <> "ROUND", Ratio, ${(parseFloat(getrules.data[i].labratiomin) + parseFloat(getrules.data[i].labratiomax)) / 2})) between ${parseFloat(getrules.data[i].labratiomin)} and ${parseFloat(getrules.data[i].labratiomax)} `
                                condition = "AND"
                            }
                            else {
                                if (typeof (req.body.ratiomin) === "number" && typeof (req.body.ratiomax) === "number") {
    ...
    removeShortlistedStone: async (req, res) => {
        try {
            if (!req.body.CustomerId) {
                return res.send({
                    success: false,
            ...
            if (!req.body.Id) {
                return res.send({ success: false, message: "Please Provide Id" });
            }            
            await QueryDB(
                `delete from CustomerShortList where Id IN (${req.body.Id?.toString()})`
            );                                    

            return res.send({
                success: true,
                message: "Shortlisted stone has been removed successfully!",
            });
        } catch (error) {
            return res.send({
            success: false,
            message: "Exception occured while removing shorlisted stone data.",
            });
        }
    },
    ...
    fetchCCModeSimilarDiamonds:async(req,res) => {
        ...
        let shapetemp = ""
            ...
            finaloutput = finaloutput.filter(val => val.STOCK_ID.toString() !== req.body.StockID.toString()).slice(0,4)
            if(finaloutput.length === 0){
                return res.send({
                    success:false,
                    message: "Stone Not Found!"
    ...
    fetchPriceRanges:async(req,res) => {
        try {
            if(!req.body.user_id){
                return res.send("Please Provide user_id!")
            }
        ...
        finalcurrency = Math.round(finalcurrency * 100)/100
            const fetchmarkups = await QueryDB(`select * from ccmode_markup where user_id = ${req.body.user_id} and rule_id = ${req.body.rule_id}`)
            if(!fetchmarkups.data.length){
                return res.send("Data Not Found!")
            }
            let fromranges = [...new Set(fetchmarkups.data.map(item => item.markupfromrange))]
            let toranges = [...new Set(fetchmarkups.data.map(item => item.markuptorange))]
            let minprice = Math.round((Math.min(...fromranges) * (1 + taxvalue/100) * finalcurrency))
            let maxprice = Math.round((Math.max(...toranges) * (1 + taxvalue/100) * finalcurrency))
            return res.send({
                minprice:minprice,
    ...
    fetchCurrency:async(req,res) => {
        try {
            if(!req.body.Currency){
                return res.send("Please Provide Currency")
            }
            const getcurrency = await QueryDB(`select * from currency_rates`)
            // console.log(getcurrency.data[0])
            let currency = 1
            delete getcurrency.data[0].base_currency
            delete getcurrency.data[0].bus_date
            delete getcurrency.data[0].updated_dt
            delete getcurrency.data[0].timestamp_val
            for(let keys in getcurrency.data[0]){
                if(!keys.includes("org")){
                    if(keys.includes(req.body.Currency.toLowerCase())){
                        currency = Math.round(getcurrency.data[0][keys] * 100)/100
                    }
                }
            }
            return res.send({currency:currency,allcurrency:getcurrency.data[0]})
        } catch (error) {
            return res.send(error)
        }
    },
    storeUserACtivity: async (req, res) => {
        try {
            if (!req.body.user_id || !req.body.activity_type || !req.body.diamond_type || !req.body.consumer_id || !req.body.ip_address || !req.body.user_agent || !req.body.activity_data) {
                return res.send({
                    success: false,
                    message: "Please Provide Proper Request Body",
                    data: null
                })
            }
            ...
            let data;
            if(req.body.activity_type === 'Ccmode Search'){
                data = convertArraysToStrings(req.body.activity_data)
            }else{
                data = req.body.activity_data
            }
            let q = `insert into ccmode_user_activity(user_id,activity_type,diamond_type,user_agent,consumer_id,ip_address,activity_data) values (
                '${req.body.user_id}','${req.body.activity_type}','${req.body.diamond_type}','${req.body.user_agent}','${req.body.consumer_id}','${req.body.ip_address}','${JSON.stringify(data)}'
            )`
            let insertquery = await QueryDB(q);
            if (!insertquery.success) {
    ...
    ccmodeBuyDiamond:async(req,res) => {
        try {
                if(!req.body.user_id){
                return res.send({
                    success:false,
        ...
        finalcurrency = Math.round(finalcurrency * 100)/100
        let shapetemp = ""
        let naturalservice = await QueryDB(`select * from nat_service_fee`)
        let labgrownservice = await QueryDB(`select * from lab_service_fee`)
            for(let i = 0; i < getrules.data.length;i++){
                if(searchquery){
                    searchquery += "UNION ALL "
                }
                ...
                let pricefrom = Math.round((typeof(req.body.total_price_from) === "number" && req.body.total_price_from >= minprice?req.body.total_price_from:minprice)/finalcurrency * 100)/100
                let priceto = Math.round((typeof(req.body.total_price_to) === "number" && req.body.total_price_to <= maxprice?req.body.total_price_to:maxprice)/finalcurrency * 100)/100
                // if(getrules.data[i].diamond_type === "L"){
                //     pricefrom = Math.round((typeof(req.body.total_price_from) === "number" && req.body.total_price_from >= minprice?req.body.total_price_from:minprice)/finalcurrency * 100)/100
                //     priceto = Math.round((typeof(req.body.total_price_to) === "number" && req.body.total_price_to <= maxprice?req.body.total_price_to:maxprice)/finalcurrency * 100)/100
                // }
                //console.log(pricefrom,priceto,"After Currency Divide")
                if(getcurrencyandtax.data[0].api_taxvalue){
                    pricefrom = pricefrom/(1 + taxvalue/100)
                ...
                let fetchservicefees
                if(getrules.data[i].diamond_type === "N"){
                    fetchservicefees = naturalservice
                }
                else{
                    fetchservicefees = labgrownservice
                }
                if(getrules.data[i].markupname === "Price"){
                    let markuprangefrom = customermarkups.find(val => pricefrom >= val.markupfromrange && pricefrom <= val.markuptorange)
                let markuprangeto = customermarkups.find(val => priceto >= val.markupfromrange && priceto <= val.markuptorange)
                ...
                let caratfrom = typeof(req.body.min_carat) === "number"?req.body.min_carat:getrules.data[i].min_carat
                let caratto = typeof(req.body.max_carat) === "number"?req.body.max_carat:getrules.data[i].max_carat
                if(getrules.data[i].diamond_type === "L"){
                    caratfrom = typeof(req.body.min_carat) === "number"?req.body.min_carat:getrules.data[i].lab_min_carat
                    caratto = typeof(req.body.max_carat) === "number"?req.body.max_carat:getrules.data[i].lab_max_carat
                }
                let caratsubquery = ``
                if(getrules.data[i].markupname === "Carat"){
                    function filterByRange(array, minRange, maxRange) {
            ...
            const fetchdata = await QueryDB(searchquery)
            let finaloutput = []
            const dianondinfo =  fetchdata.data.map(value => {
                return {
                    diamond_id:value.id,
            ...
            let inputparams = []
            if(req.body.StockID){
                inputparams = req.body.StockID.split(',')
            }
            else{
                inputparams = req.body.Certi_NO.split(',')
            }
            // const holddiamond = await holdDiamond(request,response)
            let baseurl = ""
            if(process.env.NODE_ENV === "qc"){
                if (getcountry.data[0].country.toLowerCase() === "india") {
                    baseurl = `https://api.staging.devindia.diamondsupload.com/IVP`
                }
                else if (getcountry.data[0].country.toLowerCase() === 'united kingdom' ||
                    getcountry.data[0].country.toLowerCase() === 'france' ||
                    getcountry.data[0].country.toLowerCase() === 'italy' ||
                    ...
                    getcountry.data[0].country.toLowerCase() === 'england' ||
                    getcountry.data[0].country.toLowerCase() === 'greece' || getcountry.data[0].country.toLowerCase() === 'russian federation' || getcountry.data[0].country.toLowerCase() === 'germany') {
                        baseurl = `https://api.staging.eu.diamondsupload.com/EUR`
                }
                else if(getcountry.data[0].country.toLowerCase() === 'new zealand' || getcountry.data[0].country.toLowerCase() === 'australia'){
                    baseurl = `https://api.staging.aus.diamondsupload.com/AUS`
                }
                else{
                    baseurl = `https://api.staging.global.diamondsupload.com/INTL`
                }
            }
            else{
                if (getcountry.data[0].country.toLowerCase() === "india") {
                    baseurl = `https://apiindia.diamondsoncall.com/IVP`
                }
                else if (getcountry.data[0].country.toLowerCase() === 'united kingdom' ||
                    getcountry.data[0].country.toLowerCase() === 'france' ||
                    getcountry.data[0].country.toLowerCase() === 'italy' ||
                    ...
                    getcountry.data[0].country.toLowerCase() === 'england' ||
                    getcountry.data[0].country.toLowerCase() === 'greece' || getcountry.data[0].country.toLowerCase() === 'russian federation' || getcountry.data[0].country.toLowerCase() === 'germany') {
                        baseurl = `https://api.eu.diamondsoncall.com/EUR`
                }
                else if(getcountry.data[0].country.toLowerCase() === 'new zealand' || getcountry.data[0].country.toLowerCase() === 'australia'){
                    baseurl = `https://api.aus.diamondsoncall.com/AUS`
                }
                else{
                    baseurl = `https://api.global.diamondsoncall.com/INTL`
                }
            }
            const holddiamond = await axios({
                method: "POST",
                url: `${baseurl}/HoldorBuyDiamonds`,
                headers: { "Content-Type": "application/json" },
                data: request.body,
            }).then(res => res.data)
            for(let i = 0; i < inputparams.length;i++){
                if(req.body.StockID){
                    let finddiamonds = holddiamond.data.find(val => val.diamond_id.toString() === inputparams[i].toString())
                  if(finddiamonds){
                    if(finddiamonds.status === "Diamond successfully Confirmed, Thank you"){
                        finaloutput.push({
                            Message: "Diamond(s) added in Confirm Diamond List.",
                            STOCK_ID: inputparams[i]
                        })
                    }
                    else{
                        finaloutput.push({
                            Message: "Diamond(s) not available to confirm.",
                            STOCK_ID: inputparams[i]
                        })
                      }
                  }
                    ...
                    let finddiamonds = holddiamond.data.find(val => val.Certi_NO.toString() === inputparams[i].toString())
                        if(finddiamonds){
                          if(finddiamonds.status === "Diamond successfully Confirmed, Thank you"){
                              finaloutput.push({
                                  Message: "Diamond(s) added in Confirm Diamond List.",
                                  CERTIFICATE: inputparams[i]
                              })
                          }
                          else{
                            finaloutput.push({
    ...
    loginLog: async (req, res) => {
        try {
            if (!req.body.activity_type || !req.body.ip_address || !req.body.user_agent || !req.body.activity_data) {
                return res.send({
                    success: false,
                    message: "Please Provide Proper Request Body",
                    data: null
           ...
           let q = `insert into ccmode_user_activity(activity_type,user_agent,ip_address,activity_data) values (
                '${req.body.activity_type}','${req.body.user_agent}','${req.body.ip_address}','${JSON.stringify(req.body.activity_data)}'
            )`
            let insertquery = await QueryDB(q);
            if (!insertquery.success) {
                console.log(insertquery)
    ...
    buyUserLog: async (req, res) => {
        try {
            if (!req.body.Id || !req.body.buy_ip || !req.body.buy_price || !req.body.buy_rate || !req.body.canBuy) {
                return res.send({
                    success: false,
                    message: "Please Provide Proper Request Body",
                    data: null
            ...
            let q= `UPDATE CustomerShortList
            SET buy_ip = '${req.body.buy_ip}',
